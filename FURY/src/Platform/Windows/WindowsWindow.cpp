...
namespace FURY
{
	...
	{
		FURY_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		if (!s_GLFWInitialized)
		{
			...
			FURY_CORE_ASSERT(success, "Could not initialize GLFW!");
		...
		glfwSetKeyCallback(m_Window, [](GLFWwindow* window, int key, int scancode, int action, int mods)
		{
			...
			switch (action)
			{
				case GLFW_PRESS:
				{
					FURY::KeyPressedEvent event(key, 0);
					data.EventCallback(event);
					break;
				}
				case GLFW_RELEASE:
				{
					FURY::KeyReleasedEvent event(key);
					data.EventCallback(event);
					break;
				}
				case GLFW_REPEAT:
				{
					FURY::KeyPressedEvent event(key, 1);
					data.EventCallback(event);
					break;
				}
			}
